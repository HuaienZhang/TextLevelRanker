        ...
        # for raw_sentence in np_tagged_sentences:
            ...
            for (hearst_pattern, parser) in self.__hearst_patterns:
                ...
                if matches:
                    ...
                    for j in range(len(specifics)):
                        print("Spec=%s, General=%s" % (specifics[j], general))
                        tag_sentence = self.clean_hyponym_term(sentence)
                        print(all_tagged_sentence)
                        clean_specific = specifics[j][3 : ]
                        clean_general = general[3 : ]
                        print(clean_specific)
                        print(clean_general)
                        prev_spec = ""
                        prev_general = ""
                        for k in range(0, len(all_tagged_sentence)):
                            token_tuple = all_tagged_sentence[k]
                            token = token_tuple[0]
                            word = token_tuple[1]
                            if token == clean_specific:
                                prevToken = all_tagged_sentence[k - 1]
                                if prevToken[1] == 'JJ':
                                    prev_spec = prevToken[0]
                        for k in range(0, len(all_tagged_sentence)):
                            token_tuple = all_tagged_sentence[k]
                            token = token_tuple[0]
                            word = token_tuple[1]
                            if token == clean_general:
                                prevToken = all_tagged_sentence[k - 1]
                                if prevToken[1] == 'JJ':
                                    prev_general = prevToken[0]
                        hyponyms.append((tag_sentence, prev_spec + " " + self.clean_hyponym_term(specifics[j]), prev_general + " " + self.clean_hyponym_term(general)))
